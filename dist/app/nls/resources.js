define({
  root: ({
    error: "Error",
    mapError: "Unable to create map. Item does not exist or is inaccessible.",
    licenseError: {
      message: "Your account is not licensed to use Configurable Apps that are not public. Please ask your organization administrator to assign you a user type that includes Essential Apps or an add-on Essential Apps license.",
      title: "Not Licensed"
    },
    scroll: {
      touchInstructions: "Use two-fingers to pan or zoom",
      instructions: "Press + and - keys  or use zoom controls to zoom in and out"
    },
    widgets: {
      splash: {
        tooltip: "Splash"
      },
      bookmark: {
        label: "Bookmarks"
      },
      overview: "Inset Map",
      overviewDetails: "Displays the location of the main map area in a larger scale map",
      details: {
        label: "Info"
      }
    }
  }),
  "ar": 1,
  "bs": 1,
  "ca": 1,
  "cs": 1,
  "da": 1,
  "de": 1,
  "el": 1,
  "es": 1,
  "et": 1,
  "fi": 1,
  "fr": 1,
  "he": 1,
  "hr": 1,
  "hu": 1,
  "id": 1,
  "it": 1,
  "ja": 1,
  "ko": 1,
  "lt": 1,
  "lv": 1,
  "nl": 1,
  "nb": 1,
  "pl": 1,
  "pt-br": 1,
  "pt-pt": 1,
  "ro": 1,
  "sl": 1,
  "ru": 1,
  "sr": 1,
  "sv": 1,
  "tr": 1,
  "th": 1,
  "uk": 1,
  "vi": 1,
  "zh-cn": 1,
  "zh-hk": 1,
  "zh-tw": 1
});
